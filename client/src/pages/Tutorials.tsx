...
export default function Tutorials() {
  ...
  const { data: articles = [], isLoading } = useQuery<Article[]>({
    queryKey: ["/api/articles", "tutorial"],
    queryFn: async () => {
      const res = await fetch("/api/articles?category=tutorial");
      return res.json();
    },
  });
    ...
    <div className="min-h-[100dvh] bg-black text-white">
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-xl font-bold uppercase tracking-[0.3em]">Tutorials</h1>
        </div>
        ...
        {!isLoading && articles.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-white/30 text-sm uppercase tracking-wider">No tutorials yet</p>
            <p className="text-white/20 text-xs mt-2">Check back soon for new content</p>
          </div>
        )}
        ...
        <div className="space-y-4">
          {articles.map((article, i) => (
            <motion.div
              ...
              data-testid={`card-tutorial-${article.id}`}
